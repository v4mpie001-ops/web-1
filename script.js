// =========================================
// PHẦN 1: TÀI KHOẢN & PHÂN QUYỀN (FIX LỖI DATABASE)
// =========================================
// CÀI CỨNG MÃ ĐĂNG KÝ VÀO ĐÂY (Bạn bè nhập mã này để tạo acc)
const initialValidCodes = ["VIP2026", "HTA001", "PVK999", "CODE1", "CODE2", "CODE3", "CANTHO"];

// CÀI CỨNG TÀI KHOẢN CHO BẠN BÈ VÀO ĐÂY (Không cần tạo, đăng nhập luôn)
let defaultUsers = {
  "admin": "123",
  "hta": "123",
  "pvk": "123",
  "bantoi1": "123456", // Đưa user/pass này cho bạn ông
  "bantoi2": "123456"
};
...
function handleAuth() {
  ...
  if (currentMode === 'register') {
    ...
    // Ưu tiên check trong defaultUsers (cài cứng) trước, sau đó check trong máy
    if((defaultUsers[u] && defaultUsers[u] === p) || (dbUsers[u] && dbUsers[u] === p)) {
      loginSuccess(u);
    } else {
      return showMsg("Tài khoản hoặc mật khẩu sai!");
    }
  }
}
...
// =========================================
let defaultSubjects = { "toan": { name: "Toán", icon: "📐", tests: [] }, "ly": { name: "Vật Lý", icon: "⚡", tests: [] }, "tin": { name: "Tin Học", icon: "💻", tests: [] } };
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (timeRemaining <= 0) { clearInterval(timerInterval); alert("Hết giờ!"); submitTest(); }
  }, 1000);
}
...
function renderQuestion() {
  ...
  if (q.image) {
      ...
      imgBox.innerHTML = `<img src="${q.image}" style="max-width: 100%; max-height: 400px; border-radius: 8px;">`;
  } else {
  ...
  if (q.type === 'mcq') {
    ...
    else saDiv.innerHTML = `<div>Bạn đã nhập: <b>${userAnswers[currentQIndex] || 'Trống'}</b></div><div style="color: #16a34a; margin-top:5px;">Đáp án chuẩn: <b>${q.correct}</b></div>`;
...
function submitTest() {
  ...
  const scoreDiv = document.getElementById('score-display'); scoreDiv.style.display = 'block';
  scoreDiv.innerHTML = `<div class="score-title">🎉 KẾT QUẢ: ${finalScore} / 10 Điểm 🎉</div>`;
  currentQIndex = 0; renderSidebar(); renderQuestion();
}

// =========================================
// PHẦN 4: ĐỌC FILE WORD (HỖ TRỢ ẢNH & FIX LỖI GÕ SAI)
// =========================================
...
function processWordFile() {
  ...
  if (!fileInput.files.length) return alert("Vui lòng chọn File Word!");

  const reader = new FileReader();
  reader.onload = function(e) {
    // Bật tính năng convert ảnh sang Base64 của Mammoth
    const options = {
    ...
    mammoth.convertToHtml({ arrayBuffer: e.target.result }, options)
      .then(function(result) {
        ...
        // AI TIỀN XỬ LÝ LỖI GÕ WORD CỦA BẠN:
        // 1. Ép A . thành A. (Xóa dấu cách dư thừa)
        rawHtml = rawHtml.replace(/([A-D])\s+[\.\:\)]/gi, "$1.");
        // 2. Ép xuống dòng nếu chữ A., B., C., D. dính chặt vào nhau (VD: A.30 B.45)
        rawHtml = rawHtml.replace(/([A-D][\.\:\)])/g, "<br>$1 ");
        ...
        saveSubjectsToStorage(); alert(`Nạp thành công ${parsedQuestions.length} câu!`);
...
function parseHtmlToQuestions(html) {
  ...
  // Duyệt qua từng dòng và cả Thẻ Ảnh
  tempDiv.childNodes.forEach(node => {
    if (node.nodeName === 'IMG') {
        lines.push("[IMG] " + node.src);
    } 
    else if (node.querySelectorAll) {
        // Tìm ảnh nằm lồng trong thẻ P
        node.querySelectorAll('img').forEach(img => lines.push("[IMG] " + img.src));
        
        // Thay thế thẻ <br> thành dấu phẩy đặc biệt để tách dòng chữ dính nhau
        let text = node.innerHTML.replace(/<br\s*\/?>/gi, " ||| ");
        let tempP = document.createElement('div'); tempP.innerHTML = text;
        
        tempP.textContent.split("|||").forEach(t => {
           let cleanTxt = t.trim();
           if(cleanTxt) lines.push(cleanTxt);
        });
    }
  ...
  lines.forEach(line => {
    ...
    // Đọc phần Tự Luận
    if (textUpper.includes("PHẦN III") || textUpper.includes("TRẢ LỜI NGẮN")) {
        currentSectionType = 'sa';
    } else if (textUpper.includes("PHẦN II") || textUpper.includes("ĐÚNG SAI")) {
        currentSectionType = 'tf';
    } else if (textUpper.includes("PHẦN I ") || textUpper.includes("NHIỀU PHƯƠNG ÁN")) {
    ...
    if (/^Câu\s+\d+[\.\:\-]?/i.test(line)) {
      ...
      if (line.startsWith("[IMG] ")) {
        currentQ.image = line.replace("[IMG] ", ""); // Bắt hình ảnh
      }
      else if (/^[A-D][\.\:\)]/i.test(line)) {
        currentQ.type = 'mcq';
        const key = line.charAt(0).toUpperCase();
        currentQ.options[key] = line.replace(/^[A-D][\.\:\)]\s*/i, '').trim() || "(Trống)";
      }
      else if (/^[a-d][\)\.]/i.test(line)) {
        currentQ.type = 'tf';
        const key = line.charAt(0).toLowerCase();
        currentQ.items[key] = line.replace(/^[a-d][\)\.]\s*/i, '').trim() || "(Trống)";
      }
